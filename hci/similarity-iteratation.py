...
for i_count in rag_file_count:
    ...
    for i_temp in temp:
        ...
        for i_iteration in iteration:
            gen_cq_file = f'gpt-output/rag-file-count-{i_count}/gpt-4o-temp-{i_temp}-iteration-{i_iteration}.txt'
            all_cos_result_file = f'all_cos_results/rag-file-count-{i_count}/{gpt_model}-temp-{i_temp}-{i_iteration}.csv'
            bestcq_output_file = f'rag-file-count-{i_count}/gpt-4o-temp-{i_temp}-iteration-{i_iteration}'
        ...
        print(f'metrics for temperature {i_temp}:\n{metrics_each_temp}')
        file = f'metric_results/rag-file-count-{i_count}-gpt-4o-temp-{i_temp}.txt'
        write_list_to_file(file, metrics_each_temp)
        # output file includes: P and R for (expertCQ, best_genCQ), P and R for (genCQ, best_expertCQ)	
